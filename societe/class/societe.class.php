...
class Societe extends CommonObject
{
	 ...
	 */
	public function fetch($rowid, $ref = '', $ref_ext = '', $barcode = '', $idprof1 = '', $idprof2 = '', $idprof3 = '', $idprof4 = '', $idprof5 = '', $idprof6 = '', $email = '', $ref_alias = '', $phone = '', $fax = '')
	{
		global $langs;
		global $conf;

		if (empty($rowid) && empty($ref) && empty($ref_ext) && empty($barcode) && empty($idprof1) && empty($idprof2) && empty($idprof3) && empty($idprof4) && empty($idprof5) && empty($idprof6) && empty($email) && empty($phone) && empty($fax)) {
			return -1;
		}
		...
		if ($phone) {
			$sql .= " AND (s.phone = '".$this->db->escape($phone)."'";
			$sql .= " OR s.fax = '".$this->db->escape($phone)."')";
			$sql .= " AND s.datec = (SELECT MAX(smax.datec) FROM "
				.MAIN_DB_PREFIX
				."societe as smax WHERE smax.phone = '".$this->db->escape($phone)."' "
				." OR smax.fax = '".$this->db->escape($phone)."' )";
			// $sql .= " ORDER BY date_creation desc ";
		}
		if ($fax) {
			$sql .= " AND s.fax = '".$this->db->escape($fax)."'";
			$sql .= " AND s.datec = (SELECT MAX(smax.datec) FROM "
				.MAIN_DB_PREFIX
				."societe as smax WHERE smax.fax = '".$this->db->escape($fax)."' )";
