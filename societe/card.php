...
if (is_object($objcanvas) && $objcanvas->displayCanvasExists($action)) {
	...
	if ($action == 'create') {
		...
		print '<tr style="display: none;" id="name_alias"><td><label for="name_alias_input">'.$langs->trans('AliasNames').'</label></td>';
		...
		// Prospect/Customer
		print '<tr style="display: none;"><td class="titlefieldcreate" >'.$form->editfieldkey('ProspectCustomer', 'customerprospect', '', $object, 0, 'string', '', 1).'</td>';
		...
		if (empty($conf->global->SOCIETE_DISABLE_STATE)) {
			if (!empty($conf->global->MAIN_SHOW_REGION_IN_STATE_SELECT) && ($conf->global->MAIN_SHOW_REGION_IN_STATE_SELECT == 1 || $conf->global->MAIN_SHOW_REGION_IN_STATE_SELECT == 2)) {
				print '<tr><td>'.$form->editfieldkey('District - Upazilla/Thana', 'state_id', '', $object, 0).'</td><td colspan="3" class="maxwidthonsmartphone">';
			} else {
		...
		// print '<td >Attendant Phone</td>';
		print '<td '.($conf->browser->layout == 'phone' ? ' colspan="3"' : '').'>'.img_picto('', 'object_phoning', 'class="pictofixedwidth"').' <input type="text" name="fax" id="fax" class="maxwidth200 widthcentpercentminusx" value="'.(GETPOSTISSET('fax') ?GETPOST('fax', 'alpha') : $object->fax).'"></td></tr>';
		...
		print '</tr>';
		print '<tr style="display: none;"><td>'.$form->editfieldkey('Web', 'url', '', $object, 0).'</td>';
		...
		// Vat is used
		print '<tr style="display: none;"><td>'.$form->editfieldkey('VATIsUsed', 'assujtva_value', '', $object, 0).'</td>';
		print '<td>';
		...
		print '<tr style="display: none;"><td>'.$form->editfieldkey('ThirdPartyType', 'typent_id', '', $object, 0).'</td><td class="maxwidthonsmartphone"'.( ($conf->browser->layout == 'phone' || !empty($conf->global->SOCIETE_DISABLE_WORKFORCE)) ? ' colspan="3"' : '').'>'."\n";
		...
		// Legal Form
		print '<tr style="display: none;"><td>'.$form->editfieldkey('JuridicalStatus', 'forme_juridique_code', '', $object, 0).'</td>';
		...
		if (!empty($conf->categorie->enabled) && !empty($user->rights->categorie->lire)) {
			...
			if ((!empty($conf->fournisseur->enabled) && empty($conf->global->MAIN_USE_NEW_SUPPLIERMOD)) || !empty($conf->supplier_order->enabled) || !empty($conf->supplier_invoice->enabled)) {
				print '<tr style="display: none;"  class="visibleifsupplier"><td class="toptd">'.$form->editfieldkey('SuppliersCategoriesShort', 'suppcats', '', $object, 0).'</td><td colspan="3">';
		...
		// Add logo
		print '<tr style="display: none;" class="hideonsmartphone">';
		...
		if ($socid) {
			...
			// Prospect/Customer
			print '<tr style="display: none;"><td>'.$form->editfieldkey('ProspectCustomer', 'customerprospect', '', $object, 0, 'string', '', 1).'</td>';
			print '<td class="maxwidthonsmartphone">';
			...
			if (empty($conf->global->SOCIETE_DISABLE_STATE)) {
				if (!empty($conf->global->MAIN_SHOW_REGION_IN_STATE_SELECT) && ($conf->global->MAIN_SHOW_REGION_IN_STATE_SELECT == 1 || $conf->global->MAIN_SHOW_REGION_IN_STATE_SELECT == 2)) {
					print '<tr><td>'.$form->editfieldkey('District - Upazilla/Thana', 'state_id', '', $object, 0).'</td><td colspan="3">';
				} else {
			...
			print '<td>'.img_picto('', 'object_phoning').' <input type="text" name="fax" id="fax" class="maxwidth200 widthcentpercentminusx" value="'.(GETPOSTISSET('fax') ?GETPOST('fax', 'alpha') : $object->fax).'"></td></tr>';
			...
			if (!empty($conf->categorie->enabled) && !empty($user->rights->categorie->lire)) {
				// Customer
				print '<tr ><td>'.$form->editfieldkey('CustomersCategoriesShort', 'custcats', '', $object, 0).'</td>';
				print '<td colspan="3">';
				...
				if ((!empty($conf->fournisseur->enabled) && !empty($user->rights->fournisseur->lire) && empty($conf->global->MAIN_USE_NEW_SUPPLIERMOD)) || (!empty($conf->supplier_order->enabled) && !empty($user->rights->supplier_order->lire)) || (!empty($conf->supplier_invoice->enabled) && !empty($user->rights->supplier_invoice->lire))) {
					...
					// print '<tr style="display: none;" ><td>'.$form->editfieldkey('SuppliersCategoriesShort', 'suppcats', '', $object, 0).'</td>';
					print '<td colspan="3">';
