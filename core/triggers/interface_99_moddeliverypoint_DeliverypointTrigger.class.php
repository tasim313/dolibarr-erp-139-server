...
class InterfaceDeliveryPointTrigger extends DolibarrTriggers
{
    ...
    public function runTrigger($action, $object, $user, $langs, $conf)
    {
        ...
        // Only process customer payment creation events
        ...
        try {
            // 1. Fetch invoice and payment details
            $sql = "SELECT 
                        p.amount, 
                        f.total_ttc, 
            ...
            // 2. Add custom message based on payment status
            if (isset($payment_data['invoice_paid']) && $payment_data['invoice_paid'] == 1) {
                $payment_data['status_message'] = 'abc = "Congratulations There is no due payment"';
            } else {
                $payment_data['status_message'] = 'def = "You have Due payment Please complete Your Due amount"';
            }
