...
class Facture extends CommonInvoice
{
	...
	/**
	 *    Load payments from database llx_view_invoicefields
	 *
	 *    @param	int		$id			Id of invoice to get
	 *    @return   int		            <0 if KO, 0 if not found, >0 if OK
	 */
	public function fetch_details($id)
	{
		// SELECT i.fk_source,i.fk_target,i.date_delivery,i.drname,i.note_public,i.hosname,i.civility,i.firstname,i.lastname FROM llx_view_invoicefields as i ;
		$sql = 'SELECT i.fk_source,i.fk_target,i.co_ref,i.date_delivery,i.drname,i.note_public,i.hosname,i.civility,i.firstname,i.lastname ';
		$sql .= ' FROM '.MAIN_DB_PREFIX.'view_invoicefields as i';
		if ($id > 0) {
			$sql .= ' WHERE i.fk_target = '.((int) $id);
		}
		...
		if ($resql) {
			if ($this->db->num_rows($resql)) {
				...
				$this->id             = $obj->fk_target;
				$this->fk_source      = $obj->fk_source;
				$this->date_delivery  = $obj->date_delivery;
				$this->drname_qual    = $obj->drname .' - '. $obj->note_public;
				$this->hosname        = $obj->hosname;
				$this->civility       = $obj->civility;
				$this->author         = $obj->firstname .' '. $obj->lastname;
				$this->labno         = $obj->co_ref;
