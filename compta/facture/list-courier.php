...
$search_coref = GETPOST('search_coref', 'alpha');
$search_splastname = GETPOST('search_splastname', 'alpha');
$search_wsstatus = GETPOST('search_wsstatus', 'alpha');
...
$arrayfields = array(
	...
	'co_ref'=>array('label'=>'Lab Number', 'checked'=>1, 'position'=>512, 'enabled'=>1),
	'sp_lastname'=>array('label'=>'Courier Name', 'checked'=>1, 'position'=>513, 'enabled'=>1),
	'ws_status'=>array('label'=>'WS Status', 'checked'=>1, 'position'=>514, 'enabled'=>1),
...
$sql .= ' u.login, u.lastname, u.firstname, u.email as user_email, u.statut as user_statut, u.entity, u.photo, u.office_phone, u.office_fax, u.user_mobile, u.job, u.gender, co.ref as co_ref, sp.lastname as sp_lastname, sp.phone_mobile';
...
$sqlS2 .= ' FROM '.MAIN_DB_PREFIX.'societe as s';
$sqlS2 .= " LEFT JOIN ".MAIN_DB_PREFIX."c_country as country on (country.rowid = s.fk_pays)";
$sqlS2 .= " LEFT JOIN ".MAIN_DB_PREFIX."c_typent as typent on (typent.id = s.fk_typent)";
$sqlS2 .= " LEFT JOIN ".MAIN_DB_PREFIX."c_departements as state on (state.rowid = s.fk_departement)";
if (!empty($search_categ_cus) && $search_categ_cus != '-1') {
	$sqlS2 .= ' LEFT JOIN '.MAIN_DB_PREFIX."categorie_societe as cc ON s.rowid = cc.fk_soc"; // We'll need this table joined to the select in order to filter by categ
}

$sqlS2 .= ', '.MAIN_DB_PREFIX.'facture as f';
if (is_array($extrafields->attributes[$object->table_element]['label']) && count($extrafields->attributes[$object->table_element]['label'])) {
	$sqlS2 .= " LEFT JOIN ".MAIN_DB_PREFIX.$object->table_element."_extrafields as ef on (f.rowid = ef.fk_object)";
}
if (!$sall) {
	$sqlS2 .= ' LEFT JOIN '.MAIN_DB_PREFIX.'paiement_facture as pf ON pf.fk_facture = f.rowid';
}
if ($sall || $search_product_category > 0) {
	$sqlS2 .= ' LEFT JOIN '.MAIN_DB_PREFIX.'facturedet as pd ON f.rowid=pd.fk_facture';
}
if ($search_product_category > 0) {
	$sqlS2 .= ' LEFT JOIN '.MAIN_DB_PREFIX.'categorie_product as cp ON cp.fk_product=pd.fk_product';
}
$sqlS2 .= " LEFT JOIN ".MAIN_DB_PREFIX."projet as p ON p.rowid = f.fk_projet";
$sqlS2 .= ' LEFT JOIN '.MAIN_DB_PREFIX.'user AS u ON f.fk_user_author = u.rowid';
// We'll need this table joined to the select in order to filter by sale
if ($search_sale > 0 || (empty($user->rights->societe->client->voir) && !$socid)) {
	$sqlS2 .= ", ".MAIN_DB_PREFIX."societe_commerciaux as sc";
}
if ($search_user > 0) {
	$sqlS2 .= ", ".MAIN_DB_PREFIX."element_contact as ec";
	$sqlS2 .= ", ".MAIN_DB_PREFIX."c_type_contact as tc";
}
$sqlS2 .= " LEFT JOIN llx_element_element ee on ee.fk_target = f.rowid ";
$sqlS2 .= " LEFT JOIN llx_commande co on co.rowid=ee.fk_source ";
$sqlS2 .= " LEFT JOIN llx_commande_extrafields coef on coef.fk_object = co.rowid ";
$sqlS2 .= " LEFT JOIN llx_socpeople sp on coef.courier::INTEGER = sp.rowid ";

// $sqlS2 .= " LEFT JOIN llx_commande_trackws t1 ON t1.labno=co.ref ";
// $sqlS2 .= " LEFT JOIN llx_commande_wsstatus ws ON ws.id=t1.fk_status_id ";
// $sqlS2 .= " WHERE t1.id IN (SELECT max(t2.id) ";
// $sqlS2 .= " FROM llx_commande_trackws t2 ";
// $sqlS2 .= " GROUP BY t2.labno) ";


// Add table from hooks
$parameters = array();
$reshook = $hookmanager->executeHooks('printFieldListFrom', $parameters, $object); // Note that $action and $object may have been modified by hook
$sqlS2 .= $hookmanager->resPrint;

$sqlS2 .= ' WHERE f.fk_soc = s.rowid';
$sqlS2 .= ' AND f.entity IN ('.getEntity('invoice').')';
if (empty($user->rights->societe->client->voir) && !$socid) {
	$sqlS2 .= " AND s.rowid = sc.fk_soc AND sc.fk_user = ".((int) $user->id);
}
if ($search_product_category > 0) {
	$sqlS2 .= " AND cp.fk_categorie = ".((int) $search_product_category);
}
if ($socid > 0) {
	$sqlS2 .= ' AND s.rowid = '.((int) $socid);
}
if ($userid) {
	if ($userid == -1) {
		$sqlS2 .= ' AND f.fk_user_author IS NULL';
	} else {
		$sqlS2 .= ' AND f.fk_user_author = '.((int) $userid);
	}
}
if ($search_ref) {
	$sqlS2 .= natural_search('f.ref', $search_ref);
}

if ($search_coref) {
	$sqlS2 .= natural_search('co.ref', $search_coref);
}
if ($search_splastname) {
	$sqlS2 .= natural_search('sp.lastname', $search_splastname);
}
if ($search_wsstatus) {
	$sqlS2 .= natural_search('ws.name', $search_wsstatus);
}
if ($search_refcustomer) {
	$sqlS2 .= natural_search('f.ref_client', $search_refcustomer);
}
if ($search_type != '' && $search_type != '-1') {
	$sqlS2 .= " AND f.type IN (".$db->sanitize($db->escape($search_type)).")";
}
if ($search_project_ref) {
	$sqlS2 .= natural_search('p.ref', $search_project_ref);
}
if ($search_project) {
	$sqlS2 .= natural_search('p.title', $search_project);
}
if ($search_company) {
	$sqlS2 .= natural_search('s.nom', $search_company);
}
if ($search_company_alias) {
	$sqlS2 .= natural_search('s.name_alias', $search_company_alias);
}
if ($search_town) {
	$sqlS2 .= natural_search('s.town', $search_town);
}
if ($search_zip) {
	$sqlS2 .= natural_search("s.zip", $search_zip);
}
if ($search_state) {
	$sqlS2 .= natural_search("state.nom", $search_state);
}
if (strlen(trim($search_country))) {
	...
	if ($search_country == 'special_allnotme') {
		$sqlS2 .= " AND country.code <> '".$db->escape($mysoc->country_code)."'";
	} elseif ($search_country == 'special_eec') {
		$sqlS2 .= " AND country.code IN (".$db->sanitize($country_code_in_EEC, 1).")";
	} elseif ($search_country == 'special_eecnotme') {
		$sqlS2 .= " AND country.code IN (".$db->sanitize($country_code_in_EEC_without_me, 1).")";
	} elseif ($search_country == 'special_noteec') {
		$sqlS2 .= " AND country.code NOT IN (".$db->sanitize($country_code_in_EEC, 1).")";
	} else {
		$sqlS2 .= natural_search("country.code", $search_country);
	}
}
if ($search_type_thirdparty != '' && $search_type_thirdparty != '-1') {
	$sqlS2 .= " AND s.fk_typent IN (".$db->sanitize($db->escape($search_type_thirdparty)).')';
}
if ($search_montant_ht != '') {
	$sqlS2 .= natural_search('f.total_ht', $search_montant_ht, 1);
}
if ($search_montant_vat != '') {
	$sqlS2 .= natural_search('f.total_tva', $search_montant_vat, 1);
}
if ($search_montant_localtax1 != '') {
	$sqlS2 .= natural_search('f.localtax1', $search_montant_localtax1, 1);
}
if ($search_montant_localtax2 != '') {
	$sqlS2 .= natural_search('f.localtax2', $search_montant_localtax2, 1);
}
if ($search_montant_ttc != '') {
	$sqlS2 .= natural_search('f.total_ttc', $search_montant_ttc, 1);
}
if ($search_multicurrency_code != '') {
	$sqlS2 .= " AND f.multicurrency_code = '".$db->escape($search_multicurrency_code)."'";
}
if ($search_multicurrency_tx != '') {
	$sqlS2 .= natural_search('f.multicurrency_tx', $search_multicurrency_tx, 1);
}
if ($search_multicurrency_montant_ht != '') {
	$sqlS2 .= natural_search('f.multicurrency_total_ht', $search_multicurrency_montant_ht, 1);
}
if ($search_multicurrency_montant_vat != '') {
	$sqlS2 .= natural_search('f.multicurrency_total_tva', $search_multicurrency_montant_vat, 1);
}
if ($search_multicurrency_montant_ttc != '') {
	$sqlS2 .= natural_search('f.multicurrency_total_ttc', $search_multicurrency_montant_ttc, 1);
}
if ($search_login) {
	$sqlS2 .= natural_search(array('u.login', 'u.firstname', 'u.lastname'), $search_login);
}
if ($search_categ_cus > 0) {
	$sqlS2 .= " AND cc.fk_categorie = ".((int) $search_categ_cus);
}
if ($search_categ_cus == -2) {
	$sqlS2 .= " AND cc.fk_categorie IS NULL";
}
if ($search_status != '-1' && $search_status != '') {
	if (is_numeric($search_status) && $search_status >= 0) {
		if ($search_status == '0') {
			$sqlS2 .= " AND f.fk_statut = 0"; // draft
		}
		if ($search_status == '1') {
			$sqlS2 .= " AND f.fk_statut = 1"; // unpayed
		}
		if ($search_status == '2') {
			$sqlS2 .= " AND f.fk_statut = 2"; // payed     Not that some corrupted data may contains f.fk_statut = 1 AND f.paye = 1 (it means payed too but should not happend. If yes, reopen and reclassify billed)
		}
		if ($search_status == '3') {
			$sqlS2 .= " AND f.fk_statut = 3"; // abandonned
		}
	} else {
		$sqlS2 .= " AND f.fk_statut IN (".$db->sanitize($db->escape($search_status)).")"; // When search_status is '1,2' for example
	}
}

if ($search_paymentmode > 0) {
	$sqlS2 .= " AND f.fk_mode_reglement = ".((int) $search_paymentmode);
}
if ($search_paymentterms > 0) {
	$sqlS2 .= " AND f.fk_cond_reglement = ".((int) $search_paymentterms);
}
if ($search_module_source) {
	$sqlS2 .= natural_search("f.module_source", $search_module_source);
}
if ($search_pos_source) {
	$sqlS2 .= natural_search("f.pos_source", $search_pos_source);
}
if ($search_date_start) {
	$sqlS2 .= " AND f.datef >= '".$db->idate($search_date_start)."'";
}
if ($search_date_end) {
	$sqlS2 .= " AND f.datef <= '".$db->idate($search_date_end)."'";
}
if ($search_date_valid_start) {
	$sqlS2 .= " AND f.date_valid >= '".$db->idate($search_date_valid_start)."'";
}
if ($search_date_valid_end) {
	$sqlS2 .= " AND f.date_valid <= '".$db->idate($search_date_valid_end)."'";
}
if ($search_datelimit_start) {
	$sqlS2 .= " AND f.date_lim_reglement >= '".$db->idate($search_datelimit_start)."'";
}
if ($search_datelimit_end) {
	$sqlS2 .= " AND f.date_lim_reglement <= '".$db->idate($search_datelimit_end)."'";
}
if ($option == 'late') {
	$sqlS2 .= " AND f.date_lim_reglement < '".$db->idate(dol_now() - $conf->facture->client->warning_delay)."'";
}
if ($search_sale > 0) {
	$sqlS2 .= " AND s.rowid = sc.fk_soc AND sc.fk_user = ".((int) $search_sale);
}
if ($search_user > 0) {
	$sqlS2 .= " AND ec.fk_c_type_contact = tc.rowid AND tc.element='facture' AND tc.source='internal' AND ec.element_id = f.rowid AND ec.fk_socpeople = ".((int) $search_user);
}
...
if (!$sall) {
	$sqlS2 .= ' GROUP BY co_ref,sp_lastname, sp.phone_mobile, f.rowid, f.ref, f.ref_client, f.fk_soc, f.type, f.note_private, f.note_public, f.increment, f.fk_mode_reglement, f.fk_cond_reglement, f.total_ht, f.total_tva, f.total_ttc,';
	$sqlS2 .= ' f.localtax1, f.localtax2,';
	$sqlS2 .= ' f.datef, f.date_valid, f.date_lim_reglement, f.module_source, f.pos_source,';
	$sqlS2 .= ' f.paye, f.fk_statut, f.close_code,';
	$sqlS2 .= ' f.datec, f.tms, f.date_closing,';
	$sqlS2 .= ' f.retained_warranty, f.retained_warranty_date_limit, f.situation_final, f.situation_cycle_ref, f.situation_counter,';
	$sqlS2 .= ' f.fk_user_author, f.fk_multicurrency, f.multicurrency_code, f.multicurrency_tx, f.multicurrency_total_ht,';
	$sqlS2 .= ' f.multicurrency_total_tva, f.multicurrency_total_ttc,';
	$sqlS2 .= ' s.rowid, s.nom, s.name_alias, s.email, s.phone, s.fax, s.address, s.town, s.zip, s.fk_pays, s.client, s.fournisseur, s.code_client, s.code_fournisseur, s.code_compta, s.code_compta_fournisseur,';
	$sqlS2 .= ' typent.code,';
	$sqlS2 .= ' state.code_departement, state.nom,';
	$sqlS2 .= ' country.code,';
	$sqlS2 .= " p.rowid, p.ref, p.title,";
	$sqlS2 .= " u.login, u.lastname, u.firstname, u.email, u.statut, u.entity, u.photo, u.office_phone, u.office_fax, u.user_mobile, u.job, u.gender";
	if ($search_categ_cus && $search_categ_cus != -1) {
		$sqlS2 .= ", cc.fk_categorie, cc.fk_soc";
	}
	// Add fields from extrafields
	if (!empty($extrafields->attributes[$object->table_element]['label'])) {
		foreach ($extrafields->attributes[$object->table_element]['label'] as $key => $val) {
			$sqlS2 .= ($extrafields->attributes[$object->table_element]['type'][$key] != 'separate' ? ", ef.".$key : '');
		}
	}
	...
	$sqlS2 .= natural_search(array_keys($fieldstosearchall), $sall);
}

// Add HAVING from hooks
$parameters = array();
$reshook = $hookmanager->executeHooks('printFieldListHaving', $parameters, $object); // Note that $action and $object may have been modified by hook
$sqlS2 .= empty($hookmanager->resPrint) ? "" : " HAVING 1=1 ".$hookmanager->resPrint;

$sqlS2 .= ' ORDER BY ';
$listfield = explode(',', $sortfield);
$listorder = explode(',', $sortorder);
foreach ($listfield as $key => $value) {
	$sqlS2 .= $listfield[$key].' '.($listorder[$key] ? $listorder[$key] : 'DESC').',';
}
$sqlS2 .= ' f.rowid DESC ';
...
if (empty($conf->global->MAIN_DISABLE_FULL_SCANLIST)) {
	...
	if ($sall || $search_product_category > 0 || $search_user > 0) {
		...
		$sqlforcount = 'SELECT COUNT(DISTINCT f.rowid) as nbtotalofrecords ' . $sqlS2; // FROM llx_facture as f';
	} else {
...
if ($resql) {
	...
	if ($search_coref) {
		$param .= '&search_coref='.urlencode($search_coref);
	}
	if ($search_splastname) {
		$param .= '&search_splastname='.urlencode($search_splastname);
	}
	if ($search_wsstatus) {
		$param .= '&search_wsstatus='.urlencode($search_wsstatus);
	}
	if ($search_refcustomer) {
	...
	if ($massactionbutton && $contextpage != 'poslist' && FALSE) {
	...
	if (!empty($arrayfields['co_ref']['checked'])) {
		// Lab Number
		print '<td class="liste_titre">';
		print '<input class="flat maxwidth50imp" type="text" name="search_coref" value="'.dol_escape_htmltag($search_coref).'">';
		print '</td>';
	}

	if (!empty($arrayfields['sp_lastname']['checked'])) {
		// Courier Name
		print '<td class="liste_titre">';
		print '<input class="flat" type="text" name="search_splastname" value="'.dol_escape_htmltag($search_splastname).'">';
		print '</td>';
	}

	if (!empty($arrayfields['ws_status']['checked'])) {
		// Courier Name
		print '<td class="liste_titre">';
		print '<input class="flat" type="text" name="search_wsstatus" value="'.dol_escape_htmltag($search_wsstatus).'">';
		print '</td>';
	}	
	...
	if (!empty($arrayfields['co_ref']['checked'])) {
		print_liste_field_titre($arrayfields['co_ref']['label'], $_SERVER["PHP_SELF"], "co_ref", "", $param, '', $sortfield, $sortorder, 'center nowrap ');
	}
	if (!empty($arrayfields['sp_lastname']['checked'])) {
		print_liste_field_titre($arrayfields['sp_lastname']['label'], $_SERVER["PHP_SELF"], "sp_lastname", "", $param, '', $sortfield, $sortorder, 'center nowrap ');
	}
	if (!empty($arrayfields['ws_status']['checked'])) {
		print_liste_field_titre($arrayfields['ws_status']['label'], $_SERVER["PHP_SELF"], "ws_status", "", $param, '', $sortfield, $sortorder, 'center nowrap ');
	}
	...
	if ($num > 0) {
		...
		while ($i < min($num, $limit)) {
			...
			if (!empty($arrayfields['co_ref']['checked'])) {
				print '<td class="center">';
				print dol_escape_htmltag($obj->co_ref);
				print '</td>';
				if (!$i) {
					$totalarray['nbfield']++;
				}
			...
			if (!empty($arrayfields['sp_lastname']['checked'])) {
				print '<td class="center">';
				print dol_escape_htmltag($obj->sp_lastname);
				print '</td>';
				if (!$i) {
					$totalarray['nbfield']++;
				}
			...
			if (!empty($arrayfields['ws_status']['checked'])) {
				print '<td class="center">';
				print dol_escape_htmltag($obj->wsstatus);
				print '</td>';
				if (!$i) {
					$totalarray['nbfield']++;
				}
			...
			if (($massactionbutton || $massaction) && $contextpage != 'poslist') {   // If we are in select mode (massactionbutton defined) or if we have already selected and sent an action ($massaction) defined
				...
				print '<style="display: none" input id="cb'.$obj->id.'" class="flat checkforselect" type="checkbox" name="toselect[]" value="'.$obj->id.'"'.($selected ? ' checked="checked"' : '').'>';
			}
			print '</td>';
			if (!$i) {
				$totalarray['nbfield']++;
