...
// var_dump(date("d-m-y H:i:s",$search_date_start));
// var_dump(date("d-m-y H:i:s",$search_date_end));
// var_dump($dayint);
...
$sql .= ', com.ref as com_ref, comx.test_type ';
...
$sql .= ' LEFT JOIN '.MAIN_DB_PREFIX.'element_element AS ee ON ee.fk_target = f.rowid';
$sql .= ' LEFT JOIN '.MAIN_DB_PREFIX.'commande AS com ON ee.fk_source = com.rowid';
$sql .= ' LEFT JOIN '.MAIN_DB_PREFIX.'commande_extrafields AS comx ON com.rowid = comx.fk_object';
...
if (!$sall) {
	$sql .= ' GROUP BY f.rowid, f.ref, f.ref_client, f.fk_soc, f.type, f.note_private, f.note_public, f.increment, f.fk_mode_reglement, f.fk_cond_reglement, f.total_ht, f.total_tva, f.total_ttc,';
...
if ($resql) {
	...
	if ($moreforfilter) {
		print '<div style="display:none;" class="liste_titre liste_titre_bydiv centpercent">';
		print $moreforfilter;
		print '</div>';
	}

// $arrayfields['f.ref_client']['checked']=1;
// var_dump($arrayfields);
$myafkeys = array("f.ref", "f.datef", "s.nom", "f.total_ttc", "dynamount_payed", "rtp", "u.login","f.fk_mode_reglement", "f.fk_statut");
// var_dump($myafkeys);echo "<br><br>";
foreach ($arrayfields as $afkey => $val) {
	//var_dump($tmparray);
	if (in_array($afkey, $myafkeys)) {
	// var_dump($afkey);echo "<br><br>";

		$arrayfields[$afkey]['checked'] = 1;
	} else {
		$arrayfields[$afkey]['checked'] = 0;
	}
}

	$varpage = empty($contextpage) ? $_SERVER["PHP_SELF"] : $contextpage;
	$varpage = "no_list";
// var_dump($arrayfields);echo "<br><br>";
	...
	if ($num > 0) {
		...
		while ($i < min($num, $limit)) {
			...
			if (!empty($arrayfields['f.datef']['checked'])) {
				...
				print ' '.$obj->test_type.$obj->com_ref;
				print '</td>';
				if (!$i) {
					$totalarray['nbfield']++;
				}
