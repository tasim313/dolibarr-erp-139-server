...
class Paiement extends CommonObject
{
	...
	/**
	 *    Load payments from database llx_view_payments
	 *
	...
	{
		$sql = 'SELECT p.rowid, p.payments_list, p.word_total_ttc ';
		$sql .= ' FROM '.MAIN_DB_PREFIX.'view_payments as p';
		if ($id > 0) {
		...
		if ($resql) {
			if ($this->db->num_rows($resql)) {
				...
				$this->id             = $obj->rowid;
				$this->payments_list  = $obj->payments_list.";";
				$this->word_total_ttc  = $obj->word_total_ttc;
