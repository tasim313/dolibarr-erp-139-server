...
// $search_date_startday = date('d');
// $search_date_startmonth = date('m');
// $search_date_startyear = date('Y');
// $search_date_endday = date('d');
// $search_date_endmonth = date('m');
// $search_date_endyear = date('Y');

$dayint = (GETPOST('dayint') ?GETPOST('dayint', 'int') : 0);
$search_datetime = strtotime($date .' '.$dayint.' day');

$search_date_startday = date('d', $search_datetime);
$search_date_startmonth = date('m', $search_datetime);
$search_date_startyear = date('Y', $search_datetime);
$search_date_endday = date('d', $search_datetime);
$search_date_endmonth = date('m', $search_datetime);
$search_date_endyear = date('Y', $search_datetime);
...
$arrayfields = array(
	...
	'p.datec'			=> array('label'=>"Date", 'checked'=>1, 'position'=>20),
	's.nom'				=> array('label'=>"ThirdParty", 'checked'=>1, 'position'=>30),
	'c.libelle'			=> array('label'=>"Type", 'checked'=>1, 'position'=>40),
	'transaction'		=> array('label'=>"BankTransactionLine", 'checked'=>0, 'position'=>50, 'enabled'=>(!empty($conf->banque->enabled))),
	'ba.label'			=> array('label'=>"Account", 'checked'=>1, 'position'=>60, 'enabled'=>(!empty($conf->banque->enabled))),
	'p.num_paiement'	=> array('label'=>"Numero", 'checked'=>0, 'position'=>70, 'tooltip'=>"ChequeOrTransferNumber"),
	'p.amount'			=> array('label'=>"Invoice Payment", 'checked'=>1, 'position'=>21),
	'p.amountdue'		=> array('label'=>"Due Collection", 'checked'=>1, 'position'=>22),	
	'p.statut'			=> array('label'=>"Status", 'checked'=>0, 'position'=>90, 'enabled'=>(!empty($conf->global->BILL_ADD_PAYMENT_VALIDATION))),
	'f.ref'				=> array('label'=>"RefInvoice", 'checked'=>1, 'position'=>21),
	'p.fk_user_creat'	=> array('label'=>"Author", 'checked'=>1, 'position'=>23),
	'f.datef'			=> array('label'=>"InvoiceDate", 'checked'=>1, 'position'=>100),
);
...
if (GETPOST("orphelins", "alpha")) {
	...
	$sql = "SELECT p.rowid, p.ref, p.datec, p.amount, p.statut, p.num_paiement";
	...
	$sql = "SELECT DISTINCT p.rowid, p.ref, p.datec, p.fk_bank, p.amount, p.statut, p.num_paiement, p.fk_user_creat";
	$sql .= ", c.code as paiement_code";
	...
	$sql .= ", s.rowid as socid, s.nom as name, s.email, f.ref as ref_invoice";
	$sql .= ", u.login, u.lastname, u.firstname, u.email as user_email, u.entity, u.photo, u.office_phone, u.office_fax, u.user_mobile, u.job, u.gender";
	$sql .= ", f.datef";
	...
	if ($search_date_start) {
		$sql .= " AND p.datec >= '" . $db->idate($search_date_start) . "'";
	}
	if ($search_date_end) {
		$sql .= " AND p.datec <= '" . $db->idate($search_date_end) . "'";
	}
	if ($search_account > 0) {
...
print '<tr style="display:none" class="liste_titre_filter">';
...
// 	print '<td class="liste_titre left">';
// 	print '<input class="flat" type="text" size="4" name="search_refi" value="'.dol_escape_htmltag($search_ref).'">';
// 	print '</td>';
// }
...
if (!empty($arrayfields['p.amountdue']['checked'])) {
	print_liste_field_titre($arrayfields['p.amountdue']['label'], $_SERVER["PHP_SELF"], "p.amountdue", '', $param, 'class="right"', $sortfield, $sortorder);
}
if (!empty($arrayfields['p.fk_user_creat']['checked'])) {
	print_liste_field_titre($arrayfields['p.fk_user_creat']['label'], $_SERVER["PHP_SELF"], "p.fk_user_creat", '', $param, '', $sortfield, $sortorder);
}

if (!empty($arrayfields['p.datec']['checked'])) {
	print_liste_field_titre($arrayfields['p.datec']['label'], $_SERVER["PHP_SELF"], "p.datec", '', $param, '', $sortfield, $sortorder, 'center ');
}
if (!empty($arrayfields['s.nom']['checked'])) {
...
while ($i < min($num, $limit)) {
	...
	$object->ref_invoice = $objp->ref_invoice;
	$object->fk_user_creat = $objp->fk_user_creat;
	...
	// Invoice Number f.ref
	if (!empty($arrayfields['f.ref']['checked'])) {
		print '<td>'.$object->ref_invoice.'</td>';
		if (!$i) {
			$totalarray['nbfield']++;
		}
	}	
	...
	// Amount
	if (!empty($arrayfields['p.amount']['checked'])) {
		if (dol_print_date($db->jdate($objp->datec), 'day')==dol_print_date($db->jdate($objp->datef), 'day')){
			print '<td class="right"><span class="amount">'.price($objp->amount).'</span></td>';
			if (!$i) {
				$totalarray['nbfield']++;
			}
			// $totalarray['pos'][$checkedCount] = 'amount';
			$totalarray['pos'][3] = 'amount';
			$totalarray['val']['amount'] += $objp->amount;
			$totalarray['pos'][4] = 'amountdue';
			$totalarray['val']['amountdue'] += 0;

			print '<td class="right"><span class="amountdue"></span></td>';
			if (!$i) {
				$totalarray['nbfield']++;
			}			
		}else{
			...
			print '<td class="right"><span class="amountdue">'.price($objp->amount).'</span></td>';
			if (!$i) {
				$totalarray['nbfield']++;
			}
			$totalarray['pos'][3] = 'amount';
			$totalarray['val']['amount'] += 0;			
			$totalarray['pos'][4] = 'amountdue';
			$totalarray['val']['amountdue'] += $objp->amount;

		}
	}

	// User who created payment p.fk_user_creat
	// Author 
	$userstatic->id = $objp->fk_user_creat;
	$userstatic->login = $objp->login;
	$userstatic->lastname = $objp->lastname;
	$userstatic->firstname = $objp->firstname;
	$userstatic->email = $objp->user_email;
	// $userstatic->statut = $objp->user_statut;
	$userstatic->entity = $objp->entity;
	$userstatic->photo = $objp->photo;
	$userstatic->office_phone = $objp->office_phone;
	$userstatic->office_fax = $objp->office_fax;
	$userstatic->user_mobile = $objp->user_mobile;
	...
	// if (!empty($arrayfields['p.fk_user_creat']['checked'])) {
	// 	print '<td>'.$object->fk_user_creat.'</td>';
	// 	if (!$i) {
	// 		$totalarray['nbfield']++;
	// 	}
	// }	
	...
	if (!empty($arrayfields['p.datec']['checked'])) {
		...
		print '<td class="center">'.dol_print_date($db->jdate($objp->datec), $dateformatforpayment).'</td>';
		if (!$i) {
			$totalarray['nbfield']++;
		}
	}
...
print "</form>";
		...
		// Hide sidebar and others
		$addScript =  "\n";
		$addScript .=	'<script>' . "\n";
		$addScript .=		'$(document).ready(function () {' . "\n";
		$addScript .=			'$("#mainbody").addClass("sidebar-collapse");' . "\n";
		$addScript .=			'$("#id-top").hide();' . "\n";
