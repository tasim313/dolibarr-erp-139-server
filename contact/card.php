...
if (empty($reshook)) {
	...
	if ($action == "update_extras" && !empty($permissiontoadd)) {
		...
		// $object->array_options['options_'.'fname_lname'] = 'lalalala';
...
if (is_object($objcanvas) && $objcanvas->displayCanvasExists($action)) {
		...
		if ($action == 'create') {
			...
			if (empty($conf->global->SOCIETE_DISABLE_CONTACTS)) {
				if ($socid > 0) {
					...
					print '</td></tr>';
					$hide_optionals = ' style="display: none;"';
				} else {
			...
			print '<tr'.$hide_optionals.'><td><label for="address">'.$langs->trans("Address").'</label></td>';
			...
			print '<tr'.$hide_optionals.'><td><label for="zipcode">'.$langs->trans("Zip").'</label> / <label for="town">'.$langs->trans("Town").'</label></td><td colspan="'.$colspan.'" class="maxwidthonsmartphone">';
			print $formcompany->select_ziptown((GETPOST("zipcode", 'alpha') ? GETPOST("zipcode", 'alpha') : $object->zip), 'zipcode', array('town', 'selectcountry_id', 'state_id'), 6).'&nbsp;';
			print $formcompany->select_ziptown((GETPOST("town", 'alpha') ? GETPOST("town", 'alpha') : $object->town), 'town', array('zipcode', 'selectcountry_id', 'state_id'));
			print '</td></tr>';

			// Country
			print '<tr'.$hide_optionals.'><td><label for="selectcountry_id">'.$langs->trans("Country").'</label></td><td colspan="'.$colspan.'" class="maxwidthonsmartphone">';
			...
			if (empty($conf->global->SOCIETE_DISABLE_STATE)) {
				if (!empty($conf->global->MAIN_SHOW_REGION_IN_STATE_SELECT) && ($conf->global->MAIN_SHOW_REGION_IN_STATE_SELECT == 1 || $conf->global->MAIN_SHOW_REGION_IN_STATE_SELECT == 2)) {
					print '<tr'.$hide_optionals.'><td><label for="state_id">'.$langs->trans('District - Upazilla/Thana').'</label></td><td colspan="'.$colspan.'" class="maxwidthonsmartphone">';
				} else {
					print '<tr'.$hide_optionals.'><td><label for="state_id">'.$langs->trans('State').'</label></td><td colspan="'.$colspan.'" class="maxwidthonsmartphone">';
				}
			...
			print '<td style="display: none;">'.$form->editfieldkey('PhonePerso', 'phone_perso', '', $object, 0).'</td>';
			print '<td>';
			print img_picto('', 'object_phoning');
			...
			print '<td style="display: none;">'.$form->editfieldkey('Fax', 'fax', '', $object, 0).'</td>';
			...
			if (!empty($conf->categorie->enabled) && !empty($user->rights->categorie->lire)) {
				print '<tr'.$hide_optionals.'><td>'.$form->editfieldkey('Categories', 'contcats', '', $object, 0).'</td><td colspan="3">';
			...
			// Contact by default
			if (!empty($socid)) {
				print '<tr'.$hide_optionals.'><td>'.$langs->trans("ContactByDefaultFor").'</td>';
				print '<td colspan="3">';
			...
			print load_fiche_titre('<div'.$hide_optionals.' class="comboperso">'.$langs->trans("PersonalInformations").'</div>', '', '');

			print '<table'.$hide_optionals.' class="border centpercent">';
			...
			print '<td style="display: none;"><label for="birthday_alert">'.$langs->trans("Alert").'</label>: ';
			...
			if (empty($conf->global->SOCIETE_DISABLE_STATE)) {
				if (!empty($conf->global->MAIN_SHOW_REGION_IN_STATE_SELECT) && ($conf->global->MAIN_SHOW_REGION_IN_STATE_SELECT == 1 || $conf->global->MAIN_SHOW_REGION_IN_STATE_SELECT == 2)) {
					print '<tr><td><label for="state_id">'.$langs->trans('District - Upazilla/Thana').'</label></td><td colspan="3" class="maxwidthonsmartphone">';
				} else {
			...
			// Phone
			print '<tr style="display: none;"><td>'.$form->editfieldkey('PhonePro', 'phone_pro', GETPOST('phone_pro', 'alpha'), $object, 0).'</td>';
			print '<td>';
			print img_picto('', 'object_phoning');
			...
			print '<td style="display: none;">'.$form->editfieldkey('Fax', 'fax', GETPOST('fax', 'alpha'), $object, 0).'</td>';
			print '<td style="display: none;">';
			print img_picto('', 'object_phoning_fax');
	...
	if (!empty($id) && $action != 'edit' && $action != 'create') {
		...
		print '<tr style="display: none;"><td>'.$langs->trans("ContactVisibility").'</td><td>';
