...
$usercancreatepurchaseorder = (true);
...
if (empty($reshook)) {
	$backurlforlist = DOL_URL_ROOT.'/custom/worksheets/list.php';
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = DOL_URL_ROOT.'/custom/worksheets/card.php?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}
...
if ($action == 'create' && $usercancreate) {
	...
	// Reference client
	print '<tr style="display: none;" ><td>'.$langs->trans('RefPatient').'</td><td>';
	...
	// Thirdparty
	print '<tr>';
	print '<td class="fieldrequired">'.$langs->trans('Patient').'</td>';
	if ($socid > 0) {
		print '<td>';
	...
	if ($socid > 0) {
		...
		print '<tr style="display: none;" ><td>'.$langs->trans("DefaultContact").'</td><td>';
		print img_picto('', 'contact', 'class="pictofixedwidth"');
		print $form->selectcontacts($soc->id, $contactid, 'contactid', 1, $srccontactslist, '', 1, 'maxwidth200 widthcentpercentminusx');
		print '</td></tr>';

		// Ligne info remises tiers
		print '<tr style="display: none;" ><td>'.$langs->trans('Discounts').'</td><td>';
	...
	// Delivery delay
	print '<tr style="display: none;" class="fielddeliverydelay"><td>'.$langs->trans('AvailabilityPeriod').'</td><td>';
	print img_picto('', 'clock', 'class="pictofixedwidth"');
	$form->selectAvailabilityDelay($availability_id, 'availability_id', '', 1, 'maxwidth200 widthcentpercentminusx');
	print '</td></tr>';

	// Terms of the settlement
	print '<tr style="display: none;"><td class="nowrap">'.$langs->trans('PaymentConditionsShort').'</td><td>';
	print img_picto('', 'payment', 'class="pictofixedwidth"');
	$form->select_conditions_paiements($cond_reglement_id, 'cond_reglement_id', - 1, 1, 0, 'maxwidth200 widthcentpercentminusx');
	print '</td></tr>';

	// Payment mode
	print '<tr style="display: none;"><td>'.$langs->trans('PaymentMode').'</td><td>';
	...
	print '<tr style="display: none;"><td>'.$langs->trans('Channel').'</td><td>';
	...
	if ($object->id > 0) {
		...
		print dol_get_fiche_head($head, 'order', $langs->trans("Patient Order"), -1, 'order');
		...
		*/
		...
		$linkback = '<a href="'.DOL_URL_ROOT.'/custom/worksheets/list.php?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
		...
		if (empty($conf->global->MAIN_DISABLE_OTHER_LINK) && $object->thirdparty->id > 0) {
			$morehtmlref .= ' (<a href="'.DOL_URL_ROOT.'/custom/worksheets/list.php?socid='.$object->thirdparty->id.'&search_societe='.urlencode($object->thirdparty->name).'">'.$langs->trans("OtherOrders").'</a>)';
		}
		// Project
		if (!empty($conf->projet->enabled)) {
			$langs->load("projects");
