...
class Orders extends DolibarrApi
{
	...
	/**
	 * Get tracking of an order object by ref
	 *
	 * Return an array with order informations
	 *
	 * @param       string		$ref			Ref of object
	 * @return 	array|mixed data with information
	 *
	 * @url GET    reftracking/{ref}
	 *
	 * @throws 	RestException
	 */
	public function getByRefTracking($ref)
	{
		return $this->_fetchtracking('', $ref,'');
	}	

	/**
	 * Get labno of an order object from invoice ref
	 *
	 * Return an array with order informations
	 *
	 * @param       string		$ref			Ref of object
	 * @return 	array|mixed data with information
	 *
	 * @url GET    invtracking/{ref}
	 *
	 * @throws 	RestException
	 */
	public function getByInvTracking($ref)
	{
		return $this->_fetchtracking('', '',$ref);
	}	
	...
	/**
	 * Get tracking of order objects by date
	 *
	 * Return an array with order informations
	 *
	 ...
	 * @return 	array|mixed data with information
	 *
	 * @url GET    datetracking/{scandate}/{statusid}
	 *
	 * @throws 	RestException
	 */
	public function getByDateTracking($scandate, $statusid)
	{
		return $this->_fetchdatetracking($scandate, $statusid);
	}	
	 ...
	 * @throws 	RestException
	 */
	private function _fetchdatetracking($scandate,$statusid)
	{
		if (!DolibarrApiAccess::$user->rights->commande->lire) {
			throw new RestException(401, 'Access (Get) not allowed for login '.DolibarrApiAccess::$user->login);
		}

		$errmsg='';

		$errmsg.=$scandate;
		$result = $this->commande->fetchdatetracking($scandate,$statusid);
		if (!$result) {
			throw new RestException(404, $errmsg.' Status');
		}
		return $this->_cleanObjectDatas($this->commande);
	}

	/**
	 * Get tracking info by date
	 *
	 * Return an array with order information
	 *
	 * @param       int         $id             ID of order
	 ...
	 * @throws 	RestException
	 */
	private function _fetchtracking($id, $ref = '', $invref)
	{
		if (!DolibarrApiAccess::$user->rights->commande->lire) {
			throw new RestException(401, 'Access (Get) not allowed for login '.DolibarrApiAccess::$user->login);
		}
		...
		$errmsg='';
		if($ref=='' && !empty($invref)){
			$invresult = $this->commande->fetchrefbyinv($invref);
			if($invresult){
				$ref = $this->commande->labnoref;
				$this->commande->track_invno=$invref;
				$errmsg.=$invref;
				$errmsg.=', '.$this->commande->ptname;
				if(!empty($this->commande->ptphone)){$errmsg.=', '.$this->commande->ptphone;}
				if(!empty($this->commande->ptfax)){$errmsg.=', '.$this->commande->ptfax;}
				// $errmsg.=', '.$this->commande->ptfax;
			}
		}
		if(!empty($errmsg)){ $errmsg.=', '; };
		$errmsg.=$ref;
		$result = $this->commande->fetchtracking($id, $ref);
		if (!$result) {
			throw new RestException(404, $errmsg.' Status');
		}
	...
	/**
	 * Update Status of an order (Tracking)
	 *
	 *
	 * @param   string $ref         Order REF
	 * @param   int $userid         User ID
	 * @param   int $statusid       Status ID to set
	 *
	 * @url POST    setStatus/{ref}/{userid}/{statusid}
	 *
	 ...
	 * @return  array
	 */
	public function setStatus($ref, $userid, $statusid)
	{
		if (!DolibarrApiAccess::$user->rights->commande->lire) {
			throw new RestException(401, 'Access (Set) not allowed for login '.DolibarrApiAccess::$user->login);
		}
		// $result = $this->commande->fetch($id);
		// if (!$result) {
		// 	throw new RestException(404, 'Order not found');
		...
		$result = $this->commande->set_trackingStatus(DolibarrApiAccess::$user, $ref, $userid, $statusid); 

		if ($result == 0) {
			throw new RestException(304, 'Error, nothing done.');
		}
		if ($result < 0) {
			throw new RestException(500, 'Error when setting status: '.$this->commande->error);
		}
		// $result = $this->commande->fetch($id);
