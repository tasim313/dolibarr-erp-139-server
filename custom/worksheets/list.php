...
if ($resql) {
	...
	if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $sall) {
		...
		$id = $obj->rowid;
		// header("Location: ".DOL_URL_ROOT.'/custom/worksheets/card.php?id='.$id);
		exit;
	}
	...
	$url = DOL_URL_ROOT.'/custom/worksheets/card.php?action=create';
	if (!empty($socid)) {
		$url .= '&socid='.$socid;
	}
	...
	while ($i < min($num, $limit)) {
		...
		if (!empty($arrayfields['c.ref']['checked'])) {
			...
			// Custom URL instead of getNomUrl()
			$custom_url = dol_buildpath('/custom/worksheets/card.php', 1) . '?id=' . $obj->rowid;
			
			// Add reference link with icon
			print '<a href="' . $custom_url . '">';
			print img_object($langs->trans("ShowOrder"), 'order') . ' '; // Icon before reference text
			print htmlentities($obj->ref);
			print '</a>';
			...
			if (!empty($conf->facture->enabled) && !empty($conf->global->ORDER_BILLING_ALL_CUSTOMER)) {
				if ($user->rights->facture->creer) {
					if (($obj->fk_statut > 0 && $obj->fk_statut < 3) || ($obj->fk_statut == 3 && $obj->billed == 0)) {
						print '&nbsp;<a href="'.DOL_URL_ROOT.'/custom/worksheets/list.php?socid='.$companystatic->id.'&search_billed=0&autoselectall=1">';
