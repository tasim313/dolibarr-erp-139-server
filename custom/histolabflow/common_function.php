...
function cyto_slide_prepared_list($startDate = null, $endDate = null, $dateOption = 'today'){
    global $pg_con;

    // SQL Base Query
    $baseSQL = "
        select lab_number, created_user from llx_cyto_slide_prepared
    ";
...
function cyto_special_list($startDate = null, $endDate = null, $dateOption = 'today'){
    global $pg_con;
    ...
    $baseSQL = "
        SELECT 
            ...
            s.created_user,
            s.created_date
                FROM llx_cyto_special_instructions_complete s
                JOIN llx_cyto_fixation_details f 
                  ON s.fixation_details = f.rowid
                JOIN llx_cyto c 
                ON f.cyto_id::VARCHAR = c.rowid::VARCHAR
    ";
    ...
    if ($startDate && $endDate) {
        // Case when both start and end dates are provided
        $sql = $baseSQL . "
            WHERE DATE(s.created_date) BETWEEN $1 AND $2
        ";
        $params = [$startDate, $endDate];
    } else {
        // Handling date options (today, yesterday, or both)
        switch ($dateOption) {
            case 'yesterday':
                $sql = $baseSQL . "
                    WHERE DATE(s.created_date) = CURRENT_DATE - INTERVAL '1 day'
                ";
                break;

            case 'both':
                $sql = $baseSQL . "
                    WHERE DATE(s.created_date) IN (CURRENT_DATE, CURRENT_DATE - INTERVAL '1 day')
                ";
                break;
            ...
            default:
                $sql = $baseSQL . "
                    WHERE DATE(s.created_date) = CURRENT_DATE
                ";
                break;
        }
    }
