    ...
    try {
        ...
        for ($i = 0; $i < count($specimens); $i++) {
            ...
            if (!$result_old_data || pg_num_rows($result_old_data) == 0) {
                throw new Exception("Row not found for update");
            }
            ...
            // Decode the existing previous_preliminary_report if it exists
            $previous_report_data = $previous_report ? json_decode($previous_report, true) : [];

            // Add the new update to the previous report history
            $new_report_entry = [
            ...
            // Prepare the updated JSON data for previous_preliminary_report
            $updated_previous_report = json_encode($previous_report_data);

            // Update the microscopic report table
            $sql_micro_update = "UPDATE llx_preliminary_report_microscopic
                                 SET specimen = $1, description = $2, updated_user = $3, status = $4
                                 WHERE row_id = $5";

            $params_micro = array($specimen, $description, $created_user, 'Done', $row_id);
            ...
            // Update the preliminary report table with the updated previous_preliminary_report
