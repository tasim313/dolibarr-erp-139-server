...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $updateQuery = "
        ...
        SET 
            note_public = CONCAT(
                COALESCE(note_public, ''), 
                ' ', 
                $1::text, 
                ' Added by Doctor: ', 
                $2::text, 
                ' on ', 
                TO_CHAR(NOW() AT TIME ZONE 'Asia/Dhaka', 'YYYY-MM-DD HH12:MI:SS AM')
            )
        WHERE ref = $3::text
    ";

    // Prepare and execute query with explicit types for the parameters
    $result = pg_query_params($pg_con, $updateQuery, array($note_public, $username, $ref));
