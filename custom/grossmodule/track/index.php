...
$title = $langs->trans("Gross Tracking");
$help_url = '';
...
<body>
    <div class="content">
        <form id="grossForm"  method="post" action="gross_function.php">
                ...
                <label for="gross_doctor_name">Doctor</label>
                        <select id="gross_doctor_name" name="gross_doctor_name">
                            ...
                            foreach ($doctors as $doctor) {
                                ...
                                if ($doctor['doctor_username'] == $loggedInUsername) {
                                    $selected = 'selected';
                                    $stored_doctor = isset($_SESSION['doctor_name']) && $_SESSION['doctor_name'] === $doctor['doctor_username'] ? 'selected' : '';
                                }
                ...
                <label for="gross_station_type">Gross Station</label>
                    ...
                    <label for="bones_type">Bones</label>
                    <select name="bones_type" id="bones_type" onchange="toggleTextarea()">
                        <option value="No" <?php echo !isset($_SESSION['bones_type']) || $_SESSION['bones_type'] === 'No' ? 'selected' : ''; ?>>No</option>
                        <option value="Yes" <?php echo isset($_SESSION['bones_type']) && $_SESSION['bones_type'] === 'Yes' ? 'selected' : ''; ?>>Yes</option>
                    </select>

                    <!-- Textarea that will appear when "Yes" is selected -->
                    <div id="textarea_container" style="display: <?php echo isset($_SESSION['bones_type']) && $_SESSION['bones_type'] === 'Yes' ? 'block' : 'none'; ?>;">
                        <label for="bones_description">Block Number:</label>
                        <input type="text" name="bones_description" placeholder="block number" id="bones_description" value="<?php echo isset($_SESSION['bones_description']) ? $_SESSION['bones_description'] : ''; ?>">
                        <br>
                    </div>
    ...
    <script>
            ...
            window.onload = function() {
                    const storedDoctor = sessionStorage.getItem('gross_doctor_name');
                    ...
                    if (storedDoctor) {
                        document.getElementById('gross_doctor_name').value = storedDoctor;
                    }
            ...
            document.getElementById('grossForm').addEventListener('submit', function(event) {
                const selectedDoctor = document.getElementById('gross_doctor_name').value;
                ...
                sessionStorage.setItem('gross_doctor_name', selectedDoctor);
    ...
    <script>
        function toggleTextarea() {
                var selectElement = document.getElementById("bones_type");
                var textareaContainer = document.getElementById("textarea_container");
