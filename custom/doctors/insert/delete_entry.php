        ...
        foreach ($statusChanges as $trackId => $data) {
            ...
            if ($labRoomStatus) {
                ...
                if (pg_num_rows($checkResult) > 0) {
                    ...
                    $insertQuery = pg_prepare($pg_con, "insert_query", "INSERT INTO llx_commande_trackws (labno, user_id, lab_room_status) VALUES ($1, $2, $3)");
                    if (!$insertQuery) {
                        echo json_encode(["success" => false, "message" => "Error preparing insert statement: " . pg_last_error($pg_con)]);
                        exit;
                    }
                    $insertResult = pg_execute($pg_con, "insert_query", array($labNumber, $loggedInUserId, $labRoomStatus));
