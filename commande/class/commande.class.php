...
class Commande extends CommonOrder
{
	 ...
	 *
	 * 	@param		string		$invref			Ref of invoice object
	 *	@return     int      	>0 if OK, <0 if KO, 0 if not found
	 */
	public function fetchrefbyinv($invref){
	
		// SELECT f.rowid, f.ref, e.*, s.nom, s.phone, s.fax, c.ref FROM llx_facture f
		// LEFT JOIN llx_element_element e on e.fk_target = f.rowid
		// LEFT JOIN llx_societe s on s.rowid = f.fk_soc
		// LEFT JOIN llx_commande c on c.rowid = e.fk_source
		// WHERE e.sourcetype='commande' AND e.targettype='facture'
		// AND f.ref='SI2211-0133';


		$sql = 'SELECT f.rowid, s.nom, s.phone, s.fax, c.ref ';
		$sql .= ' FROM '.MAIN_DB_PREFIX.'facture as f';
		$sql .= ' LEFT JOIN '.MAIN_DB_PREFIX.'element_element e on e.fk_target = f.rowid';
		$sql .= ' LEFT JOIN '.MAIN_DB_PREFIX.'societe s on s.rowid = f.fk_soc';
		$sql .= ' LEFT JOIN '.MAIN_DB_PREFIX.'commande c on c.rowid = e.fk_source';
		$sql .= " WHERE e.sourcetype='commande' AND e.targettype='facture'";
		$sql .= " AND f.ref='".$this->db->escape($invref)."'";
		...
		if ($result) {
			$obj = $this->db->fetch_object($result);
			if ($obj) {
				$this->labnoref = $obj->ref;
				$this->ptname = $obj->nom;
				$this->ptphone = $obj->phone;
				$this->ptfax = $obj->fax;
				return 1;
			} else {
				$this->error = 'Invoice '.$invref.' not found sql='.$sql;
				return 0;
			}
		} else {
			$this->error = $this->db->error();
	...
	/**
	 *	Get tracking info from database. Get multiple lines if available.
	 *
	 *	@param      int			$id       		Id of object to load
	 * 	@param		string		$ref			Ref of object
	 * 	@param		string		$ref_ext		External reference of object
	 ...
	 *	@return     int         				>0 if OK, <0 if KO, 0 if not found
	 */
	public function fetchtracking($id, $ref = '', $ref_ext = '', $notused = '')
	{
		// Check parameters
		if (empty($id) && empty($ref) && empty($ref_ext)) {
			return -1;
		...
		// SELECT t.create_time, t.labno, t.user_id, u.login , w.name,w.section 
		// FROM llx_commande_trackws t
		// LEFT JOIN llx_commande_wsstatus w on t.fk_status_id = w.id
		// LEFT JOIN llx_user u on t.user_id = u.rowid
		// WHERE t.labno = '2211-11223'
		// ;

		$sql = 'SELECT t.id, t.create_time, t.labno, t.user_id, u.login, w.name, w.section';
		// $sql .= ', c.amount_ht, c.total_ht, c.total_ttc, c.total_tva, c.localtax1 as total_localtax1, c.localtax2 as total_localtax2, c.fk_cond_reglement, c.fk_mode_reglement, c.fk_availability, c.fk_input_reason';
		// $sql .= ', c.fk_account';
		// $sql .= ', c.date_commande, c.date_valid, c.tms';
		// $sql .= ', c.date_livraison as delivery_date';
		...
		// $sql .= ', ca.code as availability_code, ca.label as availability_label';
		// $sql .= ', dr.code as demand_reason_code';
		$sql .= ' FROM '.MAIN_DB_PREFIX.'commande_trackws as t';
		$sql .= ' LEFT JOIN '.MAIN_DB_PREFIX.'commande_wsstatus as w ON t.fk_status_id = w.id';
		$sql .= ' LEFT JOIN '.MAIN_DB_PREFIX.'user as u ON t.user_id = u.rowid';
		...
		if ($ref) {
			$sql .= " WHERE t.labno='".$this->db->escape($ref)."'";
		}
		...
		if ($result) {
			...
			$this->track_resnum = $num;
			$i = 0;
			if ($num>0) {
				while ($i < $num) {
					$obj = $this->db->fetch_object($sql);
					// $this->expeditions[$obj->rowid] = $obj->qty;
					// t.id, t.create_time, t.labno, t.user_id, u.login , w.name,w.section
					$this->track_id[$i] = $obj->id;
					$this->track_create_time[$i] = dol_print_date($obj->create_time,"dayhour","tzserver");
					$this->track_labno[$i] = $obj->labno;
					$this->track_user_id[$i] = $obj->user_id;
					$this->track_login[$i] = $obj->login;
					$this->track_status_name[$i] = $obj->name;
					$this->track_status_section[$i] = $obj->section;
					$i++;
				}			
			// $obj = $this->db->fetch_object($result);
				...
				$this->error = 'Order with LabNo '.$ref.' not found sql='.$sql;
				return 0;
			}
		} else {
			$this->error = $this->db->error();
	...
	/**
	 *	Get tracking info from database by date. Get multiple lines if available.
	 *
	 * 	@param		string		$scandate		Date of Scan
	 * 	@param		int			$statusid		Status ID
	 *	@return     int         				>0 if OK, <0 if KO, 0 if not found
	 */
	public function fetchdatetracking($scandate,$statusid)
	{
		// Check parameters
		if (empty($scandate) && empty($statusid)) {
			return -1;
		}

		// SELECT t.create_time, t.labno, t.user_id,t.fk_status_id, u.login , w.name,w.section 
		// FROM llx_commande_trackws t
		// LEFT JOIN llx_commande_wsstatus w on t.fk_status_id = w.id
		// LEFT JOIN llx_user u on t.user_id = u.rowid
		// WHERE t.fk_status_id=3
        // AND (t.create_time BETWEEN '2022-12-10 00:00:00'::timestamp
        //          AND '2022-12-10 23:59:59'::timestamp)
        // ;
		...
		$sql .= " WHERE t.fk_status_id=".$this->db->escape($statusid);
		$sql .= " AND (t.create_time BETWEEN '".$this->db->escape($scandate)." 00:00:00'::timestamp";
		$sql .= " AND '".$this->db->escape($scandate)." 23:59:59'::timestamp)";
		...
		if ($result) {
			...
			if ($num>0) {
				...
				$this->error = 'Worksheet with Scan date '.$scandate.' not found sql='.$sql;
				return 0;
			}
		} else {
			$this->error = $this->db->error();
	...
	/**
	 *	set_trackingStatus
	 *  Adding lab no to trackws table and set status
	 *
	 *	@param     int	$ref			order ref / lab no
	 *	@param     int	$userid			User ID who set the status
	 *	@param     int	$statusid		Status ID
	 *	@return    int          		>0 si ok, <0 si ko
	 */	
	public function set_trackingStatus($user, $ref, $userid, $statusid)
	{
		global $conf, $langs;
		...
		if (!(empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->commande->lire))) {
			$this->error = 'Permission denied';
			return -1;
		}
		...
		$this->db->begin();
		// INSERT INTO llx_commande_trackws(labno,user_id,fk_status_id) VALUES('2211-12345',1,3);
		// $ref, $userid, $statusid

		// INSERT INTO llx_commande_trackws (labno,user_id,fk_status_id)
		// SELECT '2222-12345',1,3
		// WHERE
		// 	NOT EXISTS (
		// 		SELECT id FROM llx_commande_trackws WHERE user_id=1 AND fk_status_id =3
		// 	);


		$sql = "INSERT INTO ".MAIN_DB_PREFIX."commande_trackws";
		$sql .= " (labno,user_id,fk_status_id)";
		$sql .= " SELECT";
		$sql .= " '".$ref."',";
		$sql .= " ".$userid.",";
		$sql .= " ".$statusid;
		$sql .= " WHERE";
		$sql .= " NOT EXISTS(";
		$sql .= " SELECT id FROM ".MAIN_DB_PREFIX."commande_trackws";
		$sql .= " WHERE";
		$sql .= " labno='".$ref."'";
		$sql .= " AND fk_status_id=".$statusid;
		$sql .= " );";
	 ...
	 *	Get tracking info from database by date. Get multiple lines if available.
	 *
	 * 	@param		string		$ref			Lab No.
	 *	@return     int         				>0 if OK, <0 if KO, 0 if not found
	 */
	public function fetchwsstatus($ref)
	{
		// Check parameters
		if (empty($ref)) {
			return -1;
		...
		// SELECT t1.labno,(
		// 	t1.create_time at time zone 'utc' at time zone 'bdt')
		// 	,w.name FROM llx_commande_trackws t1 
		// 	LEFT JOIN llx_commande_wsstatus w ON w.id=t1.fk_status_id
		//   where t1.id in(
		//   SELECT  max(t2.id) FROM llx_commande_trackws t2
		// 	  WHERE t2.labno='2212-31333'
		// 	GROUP BY t2.labno
		// 	);

		$sql = 'SELECT t1.labno,(';
		$sql .= " 	t1.create_time at time zone 'utc' at time zone 'bdt') as scantime";
		$sql .= ' 	,w.name FROM llx_commande_trackws t1 ';
		$sql .= ' 	LEFT JOIN llx_commande_wsstatus w ON w.id=t1.fk_status_id';
		$sql .= '   where t1.id in(';
		$sql .= '		SELECT  max(t2.id) FROM llx_commande_trackws t2';
		$sql .= "  		WHERE t2.labno=".$this->db->escape($ref)."'";
		$sql .= ' 		GROUP BY t2.labno';
		$sql .= ' 	;';
		...
		if ($result) {
			...
			if ($num>0) {
				// while ($i < $num) {
					...
					// $this->expeditions[$obj->rowid] = $obj->qty;
					// t.id, t.create_time, t.labno, t.user_id, u.login , w.name,w.section
					$this->status_labno = $obj->labno;
					$this->status_scantime = dol_print_date($obj->scantime,"dayhour","tzserver");
					$this->status_name = $obj->name;
				// }			
				$this->db->free($result);
				return 1;
			} else {
				$this->error = 'Worksheet status for '.$ref.' not found sql='.$sql;
				return 0;
			}
		} else {
			$this->error = $this->db->error();
